...
def update(id,name = " ", fname = " ", mname = " ", address = " ", mobno = " ", email = " ", dob = " ", gender = " "):
       ...
       cur.execute("UPDATE student SET name = ? , fname = ? , mname = ? , address = ? , mobno = ? , email = ? , dob = ? , gender = ? where id = ?", \
                   (name, fname, mname, address , mobno, email, dob, gender,id))
       
       conn.commit()
       conn.close()

def search(id="",name = " ", fname = " ", mname = " ", address = " ", mobno = " ", email = " ", dob = " ", gender = " "):
       conn = sqlite3.connect("student.db")
       cur = conn.cursor()

       cur.execute("SELECT * FROM student WHERE id=? OR name = ? OR fname = ? OR mname = ? OR address = ? OR mobno = ? OR email = ? OR dob = ? \
                     OR gender = ?", (id,name, fname, mname, address , mobno, email, dob, gender))
       rows = cur.fetchall()
       return rows
